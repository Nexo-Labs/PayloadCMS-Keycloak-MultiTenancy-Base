...
export async function POST(request: NextRequest) {
  console.log(
    "[Logout API] ========== INICIO DEL PROCESO DE LOGOUT =========="
  );
  ...
  try {
    ...
    console.log(
      "[Logout API] 2️⃣ ID Token:",
      idToken ? `${idToken.substring(0, 20)}...` : "NO DISPONIBLE"
    );
    ...
    if (idToken) {
      ...
      console.log(
        "[Logout API] ⚠️ NO se pudo agregar id_token_hint (no disponible)"
      );
    }
    ...
    console.log(
      "[Logout API] ✅ post_logout_redirect_uri configurado:",
      postLogoutRedirectUri
    );

    console.log(
      "[Logout API] 5️⃣ URL completa de Keycloak logout:",
    ...
    console.log(
      "[Logout API] ========== FIN DEL PROCESO DE LOGOUT =========="
    );
